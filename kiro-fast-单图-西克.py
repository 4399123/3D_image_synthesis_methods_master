"""
kiro-fast-单图-西克.py —— 西克 3D（height + intensity）低延迟合成器：单图版。

与 `kiro-fast-synthesis.py` 的合成内核**完全相同**（逐像素一致），只是去掉了
批量目录扫描 / 配对 / benchmark / argparse，改成参考 `synthesize-ultra-fast-单图-西克.py`
的单图形式：改文件末尾的三个路径直接跑。

对原版 `synthesize-fast-多图-西克-去噪版.py` 的实测（本机 20 核 / OpenCV 4.8.1，
imgs2 四张图，3800x3200）：
    原版 4550 ms  ->  本版 405~482 ms（均值 452 ms），约 9~10x
    PSNR 均值 34.5 dB，SSIM 均值 0.9916
    分通道：B 完全一致（PSNR 361 dB）、R 约 52 dB、G 约 29.9 dB / SSIM 0.975

提速的五个要点（详细推导见 kiro-fast-synthesis.py 的文件头）
-----------------------------------------------------------
  A. 条带并行：OpenCV 的 morphologyEx / medianBlur / GaussianBlur / Laplacian /
     distanceTransform 全是**单线程**，setNumThreads 对它们无效。按行切条、
     每条上下留 halo（>= 算子支撑半径）后拼回，结果与整图调用**逐位一致**，
     且直接吃满多核（实测 open r=5：64 ms 单条 -> 14.5 ms 十六条）。
  B. 金字塔形态学 / LoG：形态学随半径超线性（圆盘 open r=5 → 67 ms、
     r=21 → 1041 ms）。改用 3 层金字塔各 r=5（等效 5/10/20）；归一化 LoG
     尺度不变，粗档放半分辨率算。结构元用方核（可分离，代价减半，
     G 通道 PSNR 只差 0.07 dB）。这一项把 2900 ms 压到约 90 ms。
  C. 全分辨率层不落盘：L0 的 open/close/LoG 支撑都小，与「粗层上采样 +
     归一化 + 融合 + 量化」压在同一条带内一次算完，直接出 uint8，省掉
     white/black/log_best 三个全尺寸 float32（各 27 MB）的写入与再读入；
     R 通道输入与 G 相同，一并产出（几乎免费）。
  D. 平滑场不展开：sigma_local 是 1/8 网格上的低通场，用 Lazy 保持小图，
     在条带内现场上采样，避免被读 4 次时反复搬 27 MB。
  E. 统计与低频降采样：median/MAD/分位数抽样，行偏置取 ~512 列，
     sigma=24/48/60 的掩膜感知高斯在 1/8 网格上算（原版本身也走降采样路径）。

为什么重点在减少内存流量而不是运算量：中途实测串行 1.9 s、16 条带 0.86 s，
只有 3.8x（20 核），且 CPU 时间超过串行工作量 —— 多核在等内存。流量降下来后
最优条带数才从 12 升到 14。
...
用法：
    直接运行（改文件末尾的 depth_path / png_path / output_dir）：
        python kiro-fast-单图-西克.py
    或作为模块调用：
        merged = synthesize_from_path(depth_path, png_path)
        merged = synthesize(depth16, intensity8)      # 已在内存里的数组
"""
...
def set_threads(n: int) -> None:
    """设置单图内部的条带并行度；若外层已用多进程跑多图，建议设 1。"""
    global _THREADS
...
def build_valid_mask(depth16: np.ndarray):
    ...
    返回 (raw, mask, raw_box)；raw_box 供后续求 mask 包围盒时缩小搜索范围。
    """
    h, w = depth16.shape
...
def _tophat_max(x: np.ndarray, radius: int, prev_white=None, prev_black=None):
    ...
    最终的 white - black 也不单独跑一趟，直接折进融合内核。
    """
    rows, cols = x.shape
...
def _ch_intensity(intensity8: np.ndarray, mask: np.ndarray, box) -> np.ndarray:
    ...
    **能吃多核**的算子（本机 37 ms -> 9 ms），故临时放开 OpenCV 线程。
    ...
    注意：CLAHE **必须喂整幅图**。曾尝试只在掩膜包围盒内做以省掉 45% 的像素，
    结果 tile 网格随子图尺寸改变，直方图统计窗口跟着变，B 通道对原版的 PSNR
    从「完全一致」掉到 29~33 dB，属于可见的亮度差异，故放弃。
    ...
    # 「掩膜外填均值」只需在包围盒的行范围内做——盒外整行掩膜全 0，
    # 填什么都会在最后被清零，直接复制原图即可。输出清零同理。
...
def synthesize(depth16: np.ndarray, intensity8: np.ndarray) -> np.ndarray:
    """
    height(uint16) + intensity(uint8) -> BGR uint8 三通道训练图。

    R = 局部归一化的整体形貌，G = 多尺度缺陷响应，B = 掩膜内 CLAHE 的亮度。
    """
    ...
    if box is not None:
        ...
        low = LowPass(mask_f)
        # 1) 估噪声尺度 -> 2) 只在高噪声区去噪 -> 3) 用更新后的尺度做归一化
        ...
        ch_g[sl], ch_r[sl] = _ch_defect_and_shape(res, mask_c, weight,
                                                  sigma, med0)
...
def synthesize_from_path(depth_path: str, png_path: str) -> np.ndarray:
    """读一对 <前缀>_height.tiff / <前缀>_intensity.png 并合成"""
...
if __name__ == "__main__":
    depth_path = r'./imgs/imgs2/word_3_height.tiff'
    png_path = r'./imgs/imgs2/word_3_intensity.png'
