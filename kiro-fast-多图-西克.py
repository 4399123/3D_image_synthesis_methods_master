"""
kiro-fast-synthesis.py —— 西克 3D（height + intensity）低延迟合成器。

目标：视觉效果贴近 `synthesize-fast-多图-西克-去噪版.py`，单图（3800x3200）
合成耗时压到 500 ms 以内。不修改任何原有脚本，本文件独立可跑。

原版在本机（20 核 / OpenCV 4.8.1）的耗时结构（word_4, 3800x3200）
------------------------------------------------------------------
    ch_defect(G)     2931 ms   64.8%   <- 形态学 r=5/11/21 + 3 档 LoG
    local_scale x2    532 ms   11.8%
    denoise           199 ms    4.4%
    ch_r              193 ms    4.3%
    rowbias           156 ms    3.4%   <- nanmedian 全宽
    低频/掩膜/亮度    515 ms   11.4%
    合计             4527 ms

实测（micro-benchmark，3800x3200 float32，本机）支撑本实现的三个事实：
  1. OpenCV 的 morphologyEx / medianBlur / GaussianBlur / Laplacian /
     distanceTransform 都是**单线程**，setNumThreads 对它们无效；且形态学随
     半径超线性：椭圆核 open 耗时 r=5 → 67 ms、r=11 → 319 ms、r=21 → 1041 ms。
  2. 归一化 LoG（Laplacian(Gauss(x,s)) * s^2）**尺度不变**：半分辨率上离散
     拉普拉斯放大 4 倍，s^2 缩小 4 倍，恰好抵消。形态学 Top-Hat 同理，
     半分辨率 r=5 ≈ 全分辨率 r=10。
  3. 逐像素 numpy 运算是内存带宽瓶颈（6.7M float32 一趟约 10 ms），
     `local_norm` 这种「减/除/clip/乘」链条一次要跑 5~6 趟 = 190 ms。

据此本实现做五件事（按收益排序）：
  A. 条带并行：把所有单线程算子按行切条，条带上下各留 halo（>= 算子支撑半径）
     后拼回。halo 足够时结果与整图调用**逐位一致**，且直接吃满多核。
     实测 open r=5：64 ms(1 条) → 14.5 ms(16 条)。
  B. 金字塔形态学 / LoG：Top-Hat 用 3 层金字塔各 r=5（等效 5/10/20），
     LoG 的 2.0/3.0 两档放到半分辨率上算。代价从 2900 ms 降到约 90 ms。
     Top-Hat 的结构元用方核而非圆盘：方核可分离，代价减半而 G 通道
     PSNR 只差 0.07 dB（29.01 vs 29.08）。
  C. 全分辨率层不落盘：L0 的 open/close/LoG 支撑都很小，与「粗层上采样 +
     归一化 + 融合 + 量化」压在同一条带内一次算完，直接输出 uint8。
     省掉 white/black/log_best 三个全尺寸 float32（各 27 MB）的写入与再读入。
     R 通道与 G 通道输入相同，也并进同一趟（R 几乎免费）。
     这一项把 ch_g+ch_r 从 ~460 ms 降到 ~190 ms。
  D. 平滑场不展开：sigma_local 是 1/8 网格上的低通场，用 Lazy 保持小图，
     由 _emap 在每个条带内现场上采样，避免它被 4 次读取时反复搬 27 MB。
  E. 统计与低频降采样：median/MAD/分位数按 8x8 抽样，行偏置取 ~512 列，
     sigma=24/48/60 的掩膜感知高斯在 1/8 网格上算（原版本身也走降采样路径）。

为什么重点在减少内存流量而不是减少运算量：实测串行 1.9 s、16 条带 0.86 s，
只有 3.8x 加速（20 核），且 CPU 时间超过串行工作量 —— 多核在等内存。

与原版有意保留的近似（只影响 1~2 级灰阶抖动，缺陷形态/掩膜/亮度一致）
-------------------------------------------------------------------------
  * 形态学等效半径 5/10/20（原版 5/11/21），且用方核。
  * LoG 的 2.0/3.0 两档在半分辨率上估计。
  * median/MAD/分位数抽样估计；行中位数用 512 列；G 通道的 med/MAD 用
    8 条抽样条带（约 13% 的行）估计。
  * 最大连通域的「保留哪块」判定在 1/2 网格上做（形状仍由全分辨率决定）。

实测（本机 20 核，imgs2 四张图，与原版逐图对比）
------------------------------------------------
    原版均值 4550 ms  ->  本版 470~522 ms（均值 497 ms），约 9.2x
    PSNR 均值 34.5 dB，SSIM 均值 0.9916
    分通道：B 完全一致（PSNR 361 dB）、R 约 52 dB、G 约 29.9 dB / SSIM 0.975

用法：
    python kiro-fast-synthesis.py -i ./imgs/imgs2 -o ./kiro-results
    python kiro-fast-synthesis.py -i ./imgs/imgs2 --benchmark   # 对比原版：耗时 + PSNR/SSIM
    python kiro-fast-synthesis.py -i ./imgs/imgs2 --profile     # 各阶段耗时
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="kiro 低延迟 3D 合成（height + intensity -> 3 通道训练图）")
    p.add_argument("-i", "--input", default=r"./imgs/20260813/33")
    p.add_argument("-o", "--output", default="./kiro-results")
